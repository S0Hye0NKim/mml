...
est_mml_MM <- function(max_iter, Z_list_0, Z_tilde_list_0, X_tilde_stack, Y, lamb_seq, p, adaptive = FALSE, weight, group) {
  # Setting
  n <- nrow(Y); K <- ncol(Y);
...
# Calculate degree of freedom in gBridge
cal_df_gBridge <- function(beta, lambda, group, X, gamma = 0.5, J_0, S_0) {
  tau <- (lambda * gamma^gamma)^(1/(1-gamma)) * (1-gamma)
  group_mult <- table(group) %>% as.vector() %>% sqrt
  group_mult[1] <- 0
